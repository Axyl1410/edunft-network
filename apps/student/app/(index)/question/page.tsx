...
import { CircleChevronDown, Handshake, ThumbsDown, ThumbsDownIcon, ThumbsUp } from "lucide-react"
...
export default function QuestionPage() {
  ...
  const savedQuestionObjects = questions.filter(q => savedQuestions.includes(q._id))
    ...
    <div className="container mx-auto flex min-h-svh flex-col p-4">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          {useMockData && (
            <p className="text-yellow-600 text-sm mt-1">Using mock data (API unavailable)</p>
          )}
        </div>
        <Button
          size="lg"
          ...
          {sortedQuestions.map((question) => (
            ...
            >
              <button
                ...
                aria-label={savedQuestions.includes(question._id) ? "Unsave" : "Save"}
              >
                <Bookmark
                  size={24}
                  className={savedQuestions.includes(question._id)
                    ? "text-blue-500 fill-blue-200"
                    : "text-gray-400"}
                  fill={savedQuestions.includes(question._id) ? "#bfdbfe" : "none"}
                />
              </button>
